...
export const useAnalysis = () => {
  ...
  // ... (Keep existing health check and effect logic same as before)
  const checkBackendHealth = useCallback(async (retryCount = 0): Promise<boolean> => {
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 10000); 
      const response = await fetch(`${getApiUrl()}/health`, { signal: controller.signal, method: 'GET' });
      clearTimeout(timeoutId);
      if (response.ok) { setBackendStatus('healthy'); return true; }
      else { setBackendStatus('unhealthy'); return false; }
    } catch (error) {
  ...
  // ... (Keep autosave logic same as before)
  useEffect(() => {
    ...
    setSessions(prevSessions => {
      ...
      const currentData = { files, textSources, messages, depth, updatedAt: Date.now() };
      if (index >= 0) {
  ...
  const processFile = async (file: File, fileId?: string) => {
    ...
    try {
      const response = await fetchWithRetry(`/upload/${currentSessionId}`, { method: 'POST', body: formData });
  ...
  const performDeepAnalysis = async (fileId: string) => {
    ...
    try {
        ...
        setMessages(prev => [...prev, {
            id: Date.now().toString(),
            role: 'system',
            content: `Failed to analyze ${file.name}. Ensure API key is set.`,
            timestamp: Date.now()
        }]);
  ...
  // ... (Keep existing helpers: startProcessing, addFiles, etc.)
  const startProcessing = async (fileEntries: UploadedFile[], rawFiles?: File[]) => {
    if (!config.apiKey) { alert("Please set your API Key in settings first."); return; }
    const isHealthy = await checkBackendHealth();
    if (!isHealthy) {
        setMessages(prev => [...prev, { id: Date.now().toString(), role: 'system', content: 'Backend unreachable.', timestamp: Date.now() }]);
        return;
    }
            ...
            if (fileToUpload) {
                ...
                if (result) {
                    setFiles(prev => prev.map(f => f.id === entry.id ? { 
                        ...f, status: 'analyzed', chunksProcessed: result.chunksProcessed, totalChunks: result.totalChunks, content: result.preview, analysisResults: result.analysisResults 
                    } : f));
                    delete fileObjectsRef.current[entry.id];
                    setMessages(prev => [...prev, { id: Date.now().toString(), role: 'system', content: `✅ Uploaded "${entry.name}"`, timestamp: Date.now() }]);
                }
            }
        } catch (error) {
            setFiles(prev => prev.map(f => f.id === entry.id ? { ...f, status: 'error' } : f));
        }
        setState(s => ({ ...s, progress: 10 + ((i + 1) / fileEntries.length) * 90 }));
    }
  ...
  const addFiles = useCallback(async (newFiles: File[]) => {
    ...
    const newFileEntries: UploadedFile[] = newFiles.map(file => {
        ...
        return { id, name: file.name, type: file.type, size: file.size, status: 'pending', chunksProcessed: 0, totalChunks: 0 };
    });
    setFiles(prev => [...prev, ...newFileEntries]);
    if (config.apiKey) startProcessing(newFileEntries, newFiles);
  }, [currentSessionId, config.apiKey]);

  const addFromUrl = (url: string) => { /* Same as before */ };
  const startAnalysis = () => { /* Same as before */ };
  const pauseAnalysis = () => setState(s => ({ ...s, paused: !s.paused }));
  
  const sendMessage = useCallback(async (text: string) => {
    const userMsg: Message = { id: Date.now().toString(), role: 'user', content: text, timestamp: Date.now() };
    setMessages(prev => [...prev, userMsg]);
    const historyPayload = messages.filter(m => m.role === 'user' || m.role === 'assistant').slice(-10).map(m => ({ role: m.role, content: m.content }));
    historyPayload.push({ role: 'user', content: text });
    try {
      const response = await fetchWithRetry('/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ session_id: currentSessionId, message: text, history: historyPayload }) });
      if (!response.ok) throw new Error('Backend error');
      const data = await response.json();
      setMessages(prev => [...prev, { id: (Date.now()+1).toString(), role: 'assistant', content: data.response, sources: data.sources, timestamp: Date.now() }]);
    } catch (error) {
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'system', content: 'Connection failed.', timestamp: Date.now() }]);
    }
  }, [currentSessionId, fetchWithRetry, getApiUrl, messages]);

  // Pass through other functions
  const addTextSource = (n: string, c: string) => {}; 
  ...
  const updateConfig = (u: any) => setConfig(p => ({ ...p, ...u }));
  const createSession = (name: string) => { /* Same logic */ };
  const loadSession = (id: string) => { setCurrentSessionId(id); };
  const deleteSession = (id: string) => { /* Same logic */ };
  const exportSession = (id: string) => { /* Same logic */ };
  const renameSession = (id: string, name: string) => { /* Same logic */ };
  const retryBackendConnection = async () => { checkBackendHealth(); };
