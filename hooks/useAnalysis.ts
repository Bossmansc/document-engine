...
export const useAnalysis = () => {
  ...
  const [config, setConfig] = useLocalStorage('deepseek_config', {
    apiKey: '', // NOTE: This will be sent to backend to configure the session
    autoSave: true,
    chunkSize: 1000,
    overlapSize: 200
  });
  ...
  // Sync API Key with Backend when it changes
  useEffect(() => {
    if (config.apiKey) {
  ...
  useEffect(() => {
    if (currentSessionId && sessions.length > 0) {
      ...
      if (session) {
       ...
       // Initialize a default session ID if none exists
    ...
    for (let i = 0; i < newFiles.length; i++) {
      ...
      try {
        ...
        processedFiles.push({
          ...
          type: file.type,
          size: file.size,
          status: 'analyzed', // Backend processes immediately
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: Date.now().toString(),
        role: 'system',
        content: `Error: Could not connect to backend. Is Python server running on port 8000?`,
        timestamp: Date.now()
      }]);
    }
  }, [currentSessionId]);

  // Simplified / Stubbed functions for compatibility with UI
  const addTextSource = () => {}; 
  const addFromUrl = () => {};
  const startAnalysis = () => {}; // Analysis happens on upload now
  const pauseAnalysis = () => {};
